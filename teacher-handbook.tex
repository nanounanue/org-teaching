...
\begin{NOTES}
The first level outlines can be rendered as a "separating" slides which may get a different \texttt{reveal\_background} and \texttt{class="center"} slide layout, but that isn't automatic. See \hyperref[sec:org88becc3]{Section separators}.
\end{NOTES}

\subsection{Presenter notes / content for the handbook}
\label{sec:orgf7f8e8e}
...
\label{sec:org029a73f}

\subsubsection{Babel powa}
\label{sec:org6be3895}
...
\section{Authoring}
\label{sec:org2817901}
\subsection{Modify only the lesson.org}
\label{sec:org8bb5eeb}
...
and other bits that shouldn't change much in time (see section \hyperref[sec:orgd319434]{Configuration of layout}).

\subsection{Use Emacs org-mode exporters or the Docker container}
\label{sec:orgf01af07}
...
\label{sec:org0b4482d}

Depending on how you installed org-reveal (\hyperref[sec:orgb2b40b8]{Git submodules} or otherwise), \texttt{org-reveal} may already be available.
...
\label{sec:orgb990c9c}

\subsubsection{Firefox issues ?}
\label{sec:org38c551c}
...
You may prefer to have a PDF variant of the slides (see \hyperref[sec:orgbd135aa]{Printing slides}) in case.



\section{How it works / Installation}
\label{sec:orgb1027b6}
\subsection{Use the source (Luke)}
\label{sec:orgfe32902}
...
\label{sec:org38c9bc1}

\subsubsection{Reveal.js settings}
\label{sec:org606d50c}
...
\section{Annex}
\label{sec:orgcecb0af}

\subsection{Thanks}
\label{sec:org8d8c9aa}
