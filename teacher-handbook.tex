...
\begin{NOTES}
The first level outlines can be rendered as a "separating" slides which may get a different \texttt{reveal\_background} and \texttt{class="center"} slide layout, but that isn't automatic. See \hyperref[sec:org9a9fc34]{Section separators}.
\end{NOTES}

\subsection{Presenter notes / content for the handbook}
\label{sec:org052e305}
...
\label{sec:orgfba67bb}

\subsubsection{Babel powa}
\label{sec:orgb605b5b}
...
\section{Authoring}
\label{sec:orgf08e30d}
\subsection{Modify only the lesson.org}
\label{sec:org5bca6b7}
...
and other bits that shouldn't change much in time (see section \hyperref[sec:org2131676]{Configuration of layout}).

\subsection{Use Emacs org-mode exporters or the Docker container}
\label{sec:org6e65ae2}
...
\label{sec:orgaf799df}

Depending on how you installed org-reveal (\hyperref[sec:org068e9be]{Git submodules} or otherwise), \texttt{org-reveal} may already be available.
...
\label{sec:orgf10e548}

\subsubsection{Firefox issues ?}
\label{sec:org8cf6fa3}
...
You may prefer to have a PDF variant of the slides (see \hyperref[sec:orgb0ffdf8]{Printing slides}) in case.



\section{How it works / Installation}
\label{sec:orge2f8515}
\subsection{Use the source (Luke)}
\label{sec:orgaf83c71}
...
\label{sec:orgfe98d06}

\subsubsection{Reveal.js settings}
\label{sec:org8dbeef7}
...
\section{Annex}
\label{sec:orgbfcee8f}

\subsection{Thanks}
\label{sec:org1907356}
