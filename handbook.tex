...
\mycustomhandbooktitle{How to teach using org-mode for fun and profit}{Olivier Berger}{(2018-04-23)}{org-teaching - example lesson}
\setcounter{tocdepth}{1}
\tableofcontents


\section{Introduction}
\label{sec:org7c22db1}
...
\begin{NOTES}
The first level outlines can be rendered as a "separating" slides which may get a different \texttt{reveal\_background} and \texttt{class="center"} slide layout, but that isn't automatic. See \hyperref[sec:orga93585c]{Section separators}.
\end{NOTES}

\subsection{Presenter notes / content for the handbook}
\label{sec:org6d004b7}
...
\label{sec:orgb12c1db}

\subsubsection{Babel powa}
\label{sec:org7eef586}
...
\section{Authoring}
\label{sec:org0449117}
\subsection{Modify only the lesson.org}
\label{sec:orgf54cf0f}
...
and other bits that shouldn't change much in time (see section \hyperref[sec:org24c8123]{Configuration of layout}).

\subsection{Use Emacs org-mode exporters or the Docker container}
\label{sec:org3cca3cb}
...
\label{sec:orga0df956}

Depending on how you installed org-reveal (\hyperref[sec:org8469f94]{Git submodules} or otherwise), \texttt{org-reveal} may already be available.
...
\label{sec:org652dcaa}

\subsubsection{Firefox issues ?}
\label{sec:org430a496}
...
You may prefer to have a PDF variant of the slides (see \hyperref[sec:org07a390d]{Printing slides}) in case.



\section{How it works / Installation}
\label{sec:org323a912}
\subsection{Use the source (Luke)}
\label{sec:org992a2e4}
...
\label{sec:org5c532d6}

\subsubsection{Reveal.js settings}
\label{sec:orgb68a742}
...
\section{Annex}
\label{sec:orgef7821c}

\subsection{Thanks}
\label{sec:org22150d7}
