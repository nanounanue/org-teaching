...
\begin{itemize}
\item another \textbf{handbook} for the \uline{teaching team}, to provide additional instructions (also a \href{teacher-handbook.pdf}{PDF file})
\end{itemize}
...
\begin{NOTES}
The first level outlines can be rendered as a "separating" slides which may get a different \texttt{reveal\_background} and \texttt{class="center"} slide layout, but that isn't automatic. See \hyperref[sec:org76369bd]{Section separators}.
\end{NOTES}

\subsection{Presenter notes / content for the handbook}
\label{sec:org5178282}
...
\label{sec:orgbadbb84}

\subsubsection{Babel powa}
\label{sec:orgaa0f787}
...
\section{HowTo}
\label{sec:org280f0fd}
\subsection{Use Emacs}
\label{sec:orge739aef}
...
\label{sec:org5bb1e9f}

Depending on how you installed org-reveal (\hyperref[sec:orgc1d1dc4]{Git submodules} or otherwise), \texttt{org-reveal} may already be available.
...
\label{sec:org2bdcf65}

\subsubsection{Firefox issues ?}
\label{sec:org0bdda07}
...
You may prefer to have a PDF variant of the slides (see \hyperref[sec:org9317dde]{Printing slides}) in case.



\section{How it works / Installation}
\label{sec:orgcac3ee3}
\subsection{Use the source Luke}
\label{sec:orgd725b74}
...
\label{sec:org1dbfe25}

\subsubsection{Reveal.js settings}
\label{sec:org7489d6a}
...
\section{Annex}
\label{sec:org64577bf}

\subsection{Thanks}
\label{sec:org9090d10}
