...
\begin{NOTES}
The first level outlines can be rendered as a "separating" slides which may get a different \texttt{reveal\_background} and \texttt{class="center"} slide layout, but that isn't automatic. See \hyperref[sec:org14e984e]{Section separators}.
\end{NOTES}

\subsection{Presenter notes / content for the handbook}
\label{sec:orgaf24276}
...
\label{sec:orge27693b}

\subsubsection{Babel powa}
\label{sec:org0f5edf2}
...
\section{Authoring}
\label{sec:orgd398031}
\subsection{Modify only the lesson.org}
\label{sec:orgfb49787}
...
\label{sec:org66989e0}

Depending on how you installed org-reveal (\hyperref[sec:orgbe37e54]{Git submodules} or otherwise), \texttt{org-reveal} may already be available.
...
\label{sec:org70a69bd}

\subsubsection{Firefox issues ?}
\label{sec:org7729f25}
...
You may prefer to have a PDF variant of the slides (see \hyperref[sec:org0c60b1d]{Printing slides}) in case.



\section{How it works / Installation}
\label{sec:orgf06f0b2}
\subsection{Use the source (Luke)}
\label{sec:org7e7b008}
...
\label{sec:org223a077}

\subsubsection{Reveal.js settings}
\label{sec:org51d97ca}
...
\section{Annex}
\label{sec:orgbe0ff97}

\subsection{Thanks}
\label{sec:org72581fc}
